  ...
  return (
    <Fragment>
      <div className="absolute">
        ...
        {isOpen && (
          <div className="fixed rounded-2xl bg-red-100 shadow-lg right-8 bottom-28 w-64 h-1/3">
                  ...
                  order.order.items.map((el, index) => {
                    return (
                      <div className="flex justify-between items-center">
                        ...
                        <div className="w-20 truncate">{el?.product?.productName}</div>
                        <div>#{el?.quantity}</div>
                        <div className="">{el?.totalPrice}</div>
                      </div>
                    );
                  })}
              </div>
